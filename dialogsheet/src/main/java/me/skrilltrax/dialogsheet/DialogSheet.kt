...
class DialogSheet(private val context: Context) {
    private lateinit var bottomSheetDialog: ExpandedBottomSheetDialog
    ...
    fun setIconResource(@DrawableRes icon: Int): DialogSheet {
        showIcon()
    ...
    fun setButtonsColor(@ColorInt buttonsColor: Int): DialogSheet {
        val rippleColor: Int = Utils.adjustAlpha(buttonsColor, 0.2f)
        val secondaryButtonColor = ColorStateList(arrayOf(intArrayOf(android.R.attr.state_pressed), intArrayOf(android.R.attr.state_focused), intArrayOf(android.R.attr.state_activated), intArrayOf()), intArrayOf(
                rippleColor,
                rippleColor,
                rippleColor,
                Color.TRANSPARENT
    ...
    private fun setView(view: View?): DialogSheet {
    ...
    fun show() {
        ...
        if (positiveButton.visibility != View.VISIBLE) (negativeButton.layoutParams as RelativeLayout.LayoutParams).addRule(RelativeLayout.ALIGN_PARENT_RIGHT)
        if (!areButtonsVisible()) {
            var bottomPadding = 0
            var topPadding = 0
            if (messageTextView.text != null && !TextUtils.isEmpty(messageTextView.text)) {
                bottomPadding = 24.dpToPx()
                if (titleTextView.text == null || TextUtils.isEmpty(titleTextView.text)) topPadding = 24.dpToPx()
            }
    ...
    private fun removePreviousMessageViews() {
        for (i in 1 until messageContainer.childCount) {
